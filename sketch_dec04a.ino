...
int 
      ...
      pressurePin = 1,    // Pressure pin
       pixhawkPin = 3;    // Detects drone signal

// Times and Threshold

int
        fullPower = 255,  // Maximum power level
           tLower = 7000, // Lower container time
         ...
         previous = LOW, // Previous switch state
          current = LOW, // Current switch state
            motor = LOW, // Motor direction
              led = LOW, // LED state
        collected = false; // Has the sample been collected yet
...
void loop() 
{
  ...
  //Testing for Pixhawk Voltage
  //int testValuePixHawk = analogRead(pixhawkPin);
  //Serial.println(testValuePixHawk);
  //delay(100);
